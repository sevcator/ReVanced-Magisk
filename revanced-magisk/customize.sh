...
if [ "$ARCH" = "arm" ]; then
	...
	ARCH_LIB=x86_64
else abort "! Unsupported architecture: $ARCH"; fi
...
if ! pmex path "$PKG_NAME" >&2; then
	if pmex install-existing "$PKG_NAME" >&2; then
		BASEPATH=$(pmex path "$PKG_NAME") || abort "! pm path failed $BASEPATH"
		echo >&2 "'$BASEPATH'"
		...
		if [ "${BASEPATH:1:4}" = data ]; then
			if pmex uninstall -k --user 0 "$PKG_NAME" >&2; then
				...
				abort
			else abort "! pm uninstall failed"; fi
...
install() {
	if [ ! -f "$MODPATH/$PKG_NAME.apk" ]; then
		abort "! Stock $PKG_NAME apk was not found"
	fi
	...
	for IT in 1 2; do
		if ! SES=$(pmex install-create --user 0 -i com.android.vending -r -d -S "$SZ"); then
			ui_print "! Install-create failed"
			install_err="$SES"
			break
		fi
		SES=${SES#*[} SES=${SES%]*}
		...
		if ! op=$(pmex install-commit "$SES"); then
			if echo "$op" | grep -q INSTALL_FAILED_VERSION_DOWNGRADE; then
				...
				if [ "$IS_SYS" = true ]; then
					...
					if ! op=$(pmex uninstall -k --user 0 "$PKG_NAME"); then
						ui_print "$op"
						if [ $IT = 2 ]; then
							install_err="! pm uninstall failed."
							break
						fi
					fi
					continue
			...
			ui_print "! Install-commit failed"
			install_err="$op"
			break
		fi
		if BASEPATH=$(pmex path "$PKG_NAME"); then
			...
			install_err="! Install $PKG_NAME manually and reflash the module"
			break
		fi
		break
	done
...
if [ $INS = true ] || [ -z "$(ls -A1 "$BASEPATHLIB")" ]; then
	...
	if ! op=$(unzip -o -j "$MODPATH/$PKG_NAME.apk" "lib/${ARCH_LIB}/*" -d "$BASEPATHLIB" 2>&1); then
		ui_print "! Extracting native libs failed"
		abort "$op"
	fi
